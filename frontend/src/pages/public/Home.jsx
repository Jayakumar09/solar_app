    ...
    <div className="overflow-x-hidden">
      <section className="relative flex items-center overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-6 relative z-10">
          <motion.div className="text-center max-w-4xl mx-auto" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <motion.span className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full text-white/90 text-sm font-medium mb-4 border border-white/20" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
              Trusted by 500+ Families Across India
            </motion.span>
            
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-3 leading-tight tracking-tight" style={{ textShadow: '0 4px 30px rgba(0,0,0,0.3)' }}>
              Power Your Future with <span className="text-amber-300 font-medium">Clean Energy</span>
            </h1>
            
            <motion.p className="text-base md:text-lg text-white/85 mb-5 max-w-2xl mx-auto leading-relaxed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
              Get solar installed with <span className="text-amber-300 font-semibold">ZERO upfront cost</span> using smart SISFS financing
            </motion.p>

            <motion.p className="text-amber-400 font-semibold text-base mb-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
            ...
            <motion.div className="flex flex-col sm:flex-row gap-3 justify-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
              <Link to="/book-inspection" className="group relative px-6 py-3 bg-gradient-to-r from-amber-400 to-amber-500 text-primary-900 font-bold text-base rounded-xl shadow-[0_0_30px_rgba(251,191,36,0.4)] hover:shadow-[0_0_50px_rgba(251,191,36,0.6)] hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-2 overflow-hidden">
                <span className="relative z-10 flex items-center gap-2">
                  <Play className="w-4 h-4 fill-current" /> Get Free Inspection
                </span>
                <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
              </Link>
              <Link to="/quote-request" className="px-6 py-3 bg-white/10 text-white font-medium text-base rounded-xl hover:bg-white/20 backdrop-blur-md border-2 border-white/30 hover:border-white/50 transition-all duration-300 flex items-center justify-center">
                Get Quote
              </Link>
              <Link to="/solar-calculator" className="px-6 py-3 bg-white/5 text-white font-medium text-base rounded-xl hover:bg-white/10 backdrop-blur-md border-2 border-white/10 hover:border-white/20 transition-all duration-300 flex items-center justify-center gap-2">
                <Calculator className="w-4 h-4" /> Calculator
              </Link>
            </motion.div>

            <motion.div className="mt-6 flex flex-wrap justify-center gap-4 text-white/80" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
      ...
      <section className="py-8 bg-gray-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.p className="text-center text-gray-500 text-sm font-medium uppercase tracking-widest mb-4" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
      ...
      <section className="py-10 lg:py-12 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            {trustMetrics.map(({ value, label, icon: Icon }, i) => (
              <motion.div key={label} className="group text-center p-5 lg:p-6 rounded-2xl bg-white shadow-card hover:shadow-card-hover transition-all duration-500 border border-gray-100" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}>
                <div className="w-12 h-12 mx-auto mb-3 rounded-2xl bg-gradient-to-br from-primary-100 to-primary-200 flex items-center justify-center group-hover:scale-110 transition-all duration-300 shadow-inner">
                  <Icon className="w-6 h-6 text-primary-600" />
                </div>
                <div className="text-3xl lg:text-4xl font-extrabold text-primary-700 mb-1">{value}</div>
      ...
      <section className="py-12 lg:py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-10 lg:mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <span className="text-primary-600 font-bold text-sm uppercase tracking-[0.2em]">Why Choose Us</span>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-gray-900 mt-3 mb-4 tracking-tight">Premium Features</h2>
            <p className="text-gray-500 max-w-2xl mx-auto text-lg font-light">Everything you need for a seamless transition to clean energy</p>
          </motion.div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            {features.map(({ icon: Icon, title, desc, color }, i) => (
              <motion.div key={title} className="group relative bg-white rounded-3xl p-6 shadow-card hover:shadow-card-hover transition-all duration-500 border border-gray-100 overflow-hidden" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}>
                ...
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${color} flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}>
      ...
      <section className="py-8 lg:py-10 bg-gradient-to-r from-amber-50 to-orange-50 border-y border-amber-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <p className="text-amber-700 font-semibold mb-3">🚀 Ready to start your solar journey?</p>
            <Link to="/book-inspection" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold rounded-xl hover:shadow-xl transition-all duration-300">
              Get Free Site Inspection <ArrowRight className="w-4 h-4" />
            </Link>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-12 lg:py-16 bg-gradient-to-br from-primary-900 via-primary-800 to-primary-900 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div className="text-center mb-10 lg:mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-white mt-3 mb-4 tracking-tight">How SISFS Works</h2>
            <p className="text-white/70 max-w-2xl mx-auto text-lg font-light">From request to savings in 5 simple steps</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 lg:gap-6">
            {steps.map(({ num, title, desc, icon: Icon }, i) => (
              <motion.div key={title} className="relative" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} viewport={{ once: true }}>
                <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-6 border border-white/20 hover:bg-white/15 transition-all duration-500 h-full group">
                  <div className="text-5xl font-black text-amber-400/30 mb-3 group-hover:text-amber-400/50 transition-colors">{num}</div>
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400 to-amber-500 flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
      ...
      <section className="py-12 lg:py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-10 lg:mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-gray-900 mt-3 mb-4 tracking-tight">Services & Plans</h2>
            <p className="text-gray-500 max-w-2xl mx-auto text-lg font-light">Choose the perfect solar solution for your needs</p>
          </motion.div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
            {plans.map(({ name, price, period, icon: Icon, features: planFeatures }, i) => (
              <motion.div key={name} className={`relative bg-white rounded-3xl p-6 shadow-card hover:shadow-card-hover transition-all duration-500 border-2 ${i === 1 ? 'border-primary-500 ring-4 ring-primary-500/20 scale-105' : 'border-gray-100'} group`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}>
                ...
                <div className={`w-14 h-14 rounded-2xl ${i === 1 ? 'bg-gradient-to-br from-primary-500 to-primary-600' : 'bg-gradient-to-br from-primary-100 to-primary-200'} flex items-center justify-center mb-4 group-hover:scale-110 transition-all duration-300`}>
                  <Icon className={`w-7 h-7 ${i === 1 ? 'text-white' : 'text-primary-600'}`} />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-2">{name}</h3>
                <div className="mb-6">
                  <span className="text-4xl font-extrabold text-gray-900">{price}</span>
                  <span className="text-gray-500 ml-2 text-base">/{period}</span>
                </div>
                ...
                <Link to="/book-inspection" className={`block w-full py-3 rounded-xl font-bold text-center transition-all duration-300 ${i === 1 ? 'bg-gradient-to-r from-primary-500 to-primary-600 text-white hover:shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-primary-50 hover:text-primary-700'}`}>
                  Get Started
                </Link>
              </motion.div>
            ))}
      ...
      <section className="py-8 lg:py-10 bg-gradient-to-r from-primary-50 to-emerald-50 border-y border-primary-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-primary-700 font-semibold mb-3">💰 Easy Finance Options Available</p>
            <Link to="/book-inspection" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-primary-500 to-emerald-500 text-white font-bold rounded-xl hover:shadow-xl transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-10 items-center">
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-gray-900 mt-3 mb-6 tracking-tight">Why Choose <span className="text-primary-600">Green Hybrid Power</span>?</h2>
              <p className="text-gray-500 text-lg mb-8 leading-relaxed font-light">We don&apos;t just install solar panels. We deliver complete energy independence with guaranteed savings and premium service.</p>
              <div className="space-y-4">
                {whyChooseUs.map(({ title, desc, icon: Icon }) => (
                  <div key={title} className="flex items-start gap-4 p-5 rounded-2xl bg-gray-50 hover:bg-primary-50 transition-colors group cursor-pointer">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center flex-shrink-0 shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
                      <Icon className="w-7 h-7 text-white" />
                    </div>
                    <div>
                      <h4 className="font-bold text-gray-900 text-lg mb-1">{title}</h4>
                      <p className="text-gray-500 font-light text-sm">{desc}</p>
                    </div>
                  </div>
                ))}
              </div>
              <Link to="/why-choose-us" className="inline-flex items-center mt-8 px-8 py-4 bg-gradient-to-r from-primary-500 to-primary-600 text-white font-bold rounded-2xl hover:shadow-xl transition-all duration-300">
                Learn More <ArrowRight className="w-4 h-4 ml-2" />
              </Link>
            </motion.div>
            <motion.div className="relative" initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="bg-gradient-to-br from-primary-600 to-primary-700 rounded-3xl p-6 lg:p-8 shadow-2xl">
                <div className="bg-white/10 rounded-2xl p-6 mb-6 backdrop-blur-sm">
                  <div className="text-amber-300 text-5xl font-black mb-2">95%</div>
                  <div className="text-white/90 text-lg font-medium">Customer Satisfaction Rate</div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/10 rounded-2xl p-5 backdrop-blur-sm hover:bg-white/15 transition-colors">
      ...
      <section className="py-12 lg:py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-10 lg:mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-gray-900 mt-3 mb-4 tracking-tight">What Our Customers Say</h2>
            <p className="text-gray-500 max-w-2xl mx-auto text-lg font-light">Real stories from satisfied customers across India</p>
          </motion.div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
            {testimonials.map(({ name, location, review, rating }, i) => (
              <motion.div key={name} className="bg-white rounded-3xl p-6 shadow-card hover:shadow-card-hover transition-all duration-500 border border-gray-100" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}>
                <div className="flex gap-1 mb-4">
                  {[...Array(rating)].map((_, i) => <Star key={i} className="w-5 h-5 text-amber-400 fill-amber-400" />)}
                </div>
                <p className="text-gray-600 leading-relaxed text-base mb-6 font-light">"{review}"</p>
      ...
      <section className="py-12 lg:py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-10 lg:mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-gray-900 mt-3 mb-4 tracking-tight">Frequently Asked Questions</h2>
            <p className="text-gray-500 max-w-xl mx-auto text-lg font-light">Everything you need to know about going solar</p>
          </motion.div>
          ...
          <div className="space-y-3">
            {faqs.map(({ q, a }, i) => (
              <motion.div key={i} className="border border-gray-200 rounded-2xl overflow-hidden hover:border-primary-200 transition-colors" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} viewport={{ once: true }}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full px-6 py-4 flex items-center justify-between text-left bg-white hover:bg-gray-50 transition-colors">
      ...
      <section className="py-12 lg:py-16 bg-gradient-to-br from-primary-900 via-primary-800 to-primary-900 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-white mb-6 tracking-tight">Ready to Switch to Clean Energy?</h2>
            <p className="text-white/80 text-xl mb-8 font-light">Book a free site inspection today and get a customized quotation within 24 hours.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/book-inspection" className="group px-8 py-4 bg-gradient-to-r from-amber-400 to-amber-500 text-primary-900 font-bold text-lg rounded-2xl hover:from-amber-300 hover:to-amber-400 shadow-[0_0_40px_rgba(251,191,36,0.5)] transition-all duration-300 flex items-center justify-center gap-3">
                Get Free Inspection <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to="/contact" className="px-8 py-4 bg-white/10 text-white font-semibold text-lg rounded-2xl hover:bg-white/20 backdrop-blur-md border-2 border-white/30 hover:border-white/50 transition-all duration-300 flex items-center justify-center gap-3">
